  ...
  <div>
    ...
    <Navbar />
      <div class="flex cards">
        <div class="card" 
              v-for="(lecture,index) in lectures" :key="lecture.id">
          <div class="card-img relative">
            <div class="flex absolute icons">
              <svg class="mr-3" width="18" height="23" viewBox="0 0 18 23" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <button class="" @click="addToCart(lecture.id)">
          ...
          <div class="px-6 py-4">
            ...
            <div class="flex items-center my-3">
              ...
              <div class="lecturer ml-2" 
                    v-for="teacher in lecture.lecturers" :key="teacher.id">
                {{ teacher.username }}
              </div>
            </div>
            <div class="flex justify-between text-gray-700 text-base mb-1">
              <span class="funding">剩3天</span>
              <span class="funding">
                已募資{{ getProgress(index) }}%
              </span>
            </div>
            <!-- <div class="mb-1 text-base font-medium text-red-700 dark:text-red-500"></div> -->
            <div class="w-full bg-gray-200 rounded-full h-2.5 dark:bg-gray-700">
              <div class="percentage h-2.5 rounded-full dark:bg-red-500" :style="{ 'width': getProgress(index)+'%'}">
              </div>
            </div>
          </div>
          <div class="flex items-center pl-3">
            <span class="on-sale pl-3 text-gray-700 font-medium">$ 
              {{ lecture.price }}
            </span>
            <span class="price pl-3 line-through">$ 
              {{ lecture.fixed_price }}
            </span>
          </div>
        </div>
...
export default {
  ...
  methods: {
    addToCart(id) {
      this.itemInCart.items[0].id = id;
      this.hiskio_token = document.cookie.split('=')[1];
      console.log(`this.hiskio_token --> ${this.hiskio_token}`);
      fetch('https://api.hiskio.com/v2/carts', {
        ...
        body: JSON.stringify(this.itemInCart),
      })
